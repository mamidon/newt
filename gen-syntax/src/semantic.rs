use crate::parse::{ParseError, ParseErrorKind, SyntaxNode};
...
impl<'a> SemanticsContext<'a> {
    ...
    fn define_symbol(&mut self, token: Token) -> Option<ParseError> {
        ...
        if !self.symbols.contains_key(lexeme) {
            ...
            Some(ParseError::new(
                token,
                ParseErrorKind::DuplicateSymbol {
                    symbol: lexeme.to_string(),
                },
            ))
        }
    }
...
pub fn validate_semantics(
    root: &SyntaxNode,
    source: &str,
) -> Result<CodeGenContext, Vec<ParseError>> {
...
fn build_code_gen_context(context: &mut SemanticsContext) -> CodeGenContext {
    let symbols: Vec<Symbol> = context
        .symbols
        .drain()
        .into_iter()
        .map(|tuple| tuple.1)
    ...
    for production in root.iter() {
        if let SyntaxNode::Pipe(options) = production {
            for option in options.iter() {
                let identifier_tokens: Vec<Token> = option
                    .iter()
                ...
                let unique_identifiers: HashSet<String> = identifier_tokens
                    .iter()
                ...
                if unique_identifiers.len() > 1 {
                    ...
                    errors.push(ParseError::new(
                        *identifier_tokens.first().unwrap(),
                        error_kind,
                    ))
                }
            }
