...
use crate::featurez::syntax::{
    NewtResult, NewtRuntimeError, NewtString, NewtValue, SyntaxKind, SyntaxTree,
};
use crate::featurez::tokenize;
use crate::featurez::{StrTokenSource, VirtualMachine};
...
fn return_statement_short_circuits() {
    ...
    assert_eq!(
        Ok(NewtValue::Int(42)),
        vm.interpret("return_short_circuits()")
    );
}
...
fn if_else_statement_executes_correct_branches_for_conditional() {
    ...
    assert_eq!(
        Ok(NewtValue::Int(1)),
        vm.interpret("if_else_statement(true)")
    );
    assert_eq!(
        Ok(NewtValue::Int(2)),
        vm.interpret("if_else_statement(false)")
    );
}
...
fn while_statement_uses_truthy_semantics() {
    ...
    assert_eq!(
        Ok(NewtValue::Bool(true)),
        vm.interpret("while_statement(1)")
    );
    assert_eq!(
        Ok(NewtValue::Bool(false)),
        vm.interpret("while_statement(0)")
    );
}
...
fn newt_value_truthy_semantics_for_untruthy_values() {
    ...
    assert_eq!(
        Err(NewtRuntimeError::TypeError),
        vm.interpret("truthiness(1.0)")
    );
    assert_eq!(
        Err(NewtRuntimeError::TypeError),
        vm.interpret("truthiness(\"foo\")")
    );
    assert_eq!(
        Err(NewtRuntimeError::TypeError),
        vm.interpret("truthiness('c')")
    );
    assert_eq!(
        Err(NewtRuntimeError::TypeError),
        vm.interpret("truthiness(truthiness)")
    );
    assert_eq!(
        Err(NewtRuntimeError::TypeError),
        vm.interpret("truthiness(null_value())")
    );
}
...
fn newt_value_string_equality_semantics() {
    ...
    assert_eq!(
        Ok(NewtValue::Bool(true)),
        vm.interpret("\"Hello, world!\" == \"Hello, world!\"")
    );
    assert_eq!(
        Ok(NewtValue::Bool(false)),
...
fn function_declaration_statement_adds_function_to_scope() {
    ...
    assert_eq!(
        Ok(NewtValue::Int(42)),
        vm.interpret("function_declaration()")
    );
    assert_eq!(
...
fn function_declaration_statement_can_nest_declarations() {
    ...
    assert_eq!(
        Ok(NewtValue::Int(32)),
        vm.interpret("outer_declaration()()")
    );
}
...
fn object_literals_are_correctly_evaluated_with_multiple_properties() {
    ...
    assert_eq!(Some(NewtValue::Int(42)), object.get("x"));
    assert_eq!(
        Some(NewtValue::String(NewtString::new("hello, world"))),
        object.get("y")
    );
...
fn object_literals_are_correctly_evaluated_with_complex_properties() {
    ...
    assert_eq!(
        Ok(NewtValue::Int(42)),
        vm.interpret("object_function().x()")
    );
}
...
fn virtual_machine_short_circuits_incorrect_syntax() {
    ...
    assert_eq!(
        Err(NewtRuntimeError::InvalidSyntaxTree),
        vm.interpret("2++2")
    );
}
