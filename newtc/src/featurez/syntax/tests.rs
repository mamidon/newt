...
fn return_stmt_node_can_round_trip_as_stmt_node() {
    ...
    let stmt_node = StmtNode::cast(return_node.to_inner()).expect("Valid StmtNode");
...
fn function_declaration_stmt_node_round_trips() {
    ...
    let stmt_node = StmtNode::cast(function_node.to_inner()).expect("Valid StmtNode");
...
fn while_stmt_node_handles_conditional() {
    ...
    let conditional_kind = while_node.condition().syntax().nth_node(0).kind();
...
fn while_stmt_node_handles_stmts() {
    ...
    let stmt_count = while_node.stmts().stmts().count();
...
fn while_stmt_node_round_trips() {
    ...
    let stmt_node = StmtNode::cast(while_node.to_inner()).expect("Valid StmtNode");
...
fn if_stmt_node_handles_conditional() {
    ...
    let conditional_kind = node.condition().syntax().nth_node(0).kind();
...
fn if_stmt_node_handles_when_true_stmts() {
    ...
    let stmt_count = node.when_true().stmts().count();
...
fn if_stmt_node_handles_when_false_stmts() {
    ...
    let stmt_count = node.when_false().unwrap().stmts().count();
...
fn if_stmt_node_does_not_swap_true_and_false_branches() {
    let tree: SyntaxTree = "if(true) \
                            { let x = 1; } \
                            else { let y = 2; let z = 3; }"
        .into();
    let node: &IfStmtNode = expect_stmt_node(&tree);

    let true_stmt_count = node.when_true().stmts().count();

    let false_stmt_count = node.when_false().unwrap().stmts().count();
...
fn if_stmt_node_round_trips() {
    ...
    let stmt_node = StmtNode::cast(node.to_inner()).expect("Valid StmtNode");
...
fn object_literal_expr_node_handles_one_field() {
    ...
    assert_eq!(
        Some(SyntaxKind::PrimitiveLiteralExpr),
        node.fields().get("bar").map(|e| e.to_inner().kind())
    );
}
...
fn object_literal_expr_node_handles_multiple_fields() {
    ...
    let fields: HashMap<String, SyntaxKind> = node
        .fields()
        .into_iter()
    ...
    let inner_fields = match outer_node.fields().get("bar") {
        Some(expr) => match expr.kind() {
            ExprKind::ObjectLiteralExpr(literal) => {
                let bar_fields: HashMap<String, SyntaxKind> = literal
                    .fields()
                    .into_iter()
                    .map(|(k, v)| (k, v.to_inner().kind()))
                    .collect();
...
fn object_property_expr_handles_multiple_property_access() {
    ...
    let buzz_expr: &ObjectPropertyExprNode =
        ObjectPropertyExprNode::from_inner(foo_expr.source_expr().to_inner());
    let fizz_expr: &VariableExprNode =
        VariableExprNode::from_inner(buzz_expr.source_expr().to_inner());
...
fn unary_expr_node_handles_operators() {
    ...
    let test_cases = [("!true", TokenKind::Bang), ("-2", TokenKind::Minus)];
